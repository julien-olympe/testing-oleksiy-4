...
test.describe('Critical Path E2E Test - Complete Happy Path', () => {
  ...
  test('CP-001: Complete Happy Path - User Registration to Function Execution', async ({ page }) => {
    ...
    await test.step('Step 12: Set Brick Input Parameter', async () => {
      ...
      const updateResponse = await updateBrickPromise;
      
      // Verify the update was successful
      if (updateResponse.status() !== 200) {
        const errorBody = await updateResponse.text().catch(() => '');
        throw new Error(`Failed to update brick configuration: Status ${updateResponse.status()}, ${errorBody}`);
      }
      
      // Verify the response contains the updated configuration
      try {
    ...
    await test.step('Step 14: Run Function', async () => {
      test.setTimeout(120000); // Increase timeout for this step
      
      // Wait a bit to ensure Step 13 completed and page is stable
      await page.waitForTimeout(1000);
      ...
      // (This was already verified in Step 12, so we can skip the UI check here)
      // If database wasn't set, the execution would fail anyway
      ...
      // (Already verified above with edges count)
      
      // 5. Verify RUN button is visible above the search bar in the left side panel
      let runButton = page.locator('button:has-text("RUN"), button:has-text("Run")');
      ...
      // Verify nodes and edges are still present after reload
      const nodesAfterReload = page.locator('.react-flow__node');
      await expect(nodesAfterReload).toHaveCount(3, { timeout: 10000 });
      
      const edgesAfterReload = page.locator('.react-flow__edge');
      await expect(edgesAfterReload).toHaveCount(2, { timeout: 10000 });
      
      // Clear console logs before execution (re-initialize console log capture after reload)
      consoleLogs = [];
      ...
      if (runResponse.status() !== 200) {
        let responseBody = '';
        try {
          responseBody = await runResponse.text();
        } catch (e) {
        ...
        try {
          ...
          if (errorJson.error && typeof errorJson.error === 'object') {
            ...
            if (errorJson.error.details && Object.keys(errorJson.error.details).length > 0) {
              errorDetails += ` - Details: ${JSON.stringify(errorJson.error.details, null, 2)}`;
            }
            if (errorJson.error.code) {
              errorDetails = `${errorJson.error.code}: ${errorDetails}`;
            }
          } else if (errorJson.code && errorJson.message) {
            // Alternative format: { code, message, details }
            errorDetails = errorJson.message;
            if (errorJson.details && Object.keys(errorJson.details).length > 0) {
              errorDetails += ` - Details: ${JSON.stringify(errorJson.details, null, 2)}`;
            }
            errorDetails = `${errorJson.code}: ${errorDetails}`;
        ...
        throw new Error(`Function execution failed with status ${runResponse.status()}: ${errorDetails}`);
      }
      
      // Parse API response to get consoleOutput
      const responseBody = await runResponse.json();
      const apiConsoleOutput = responseBody.execution?.consoleOutput || [];
      ...
      // (This is verified by checking console output contains expected values)
      
      // Wait for console logs to be captured (execution may take some time)
      // Frontend logs API consoleOutput to browser console, so wait for that
      await page.waitForTimeout(3000);
      ...
      // Check both API response consoleOutput and browser console logs
      const allConsoleOutput = [...apiConsoleOutput, ...consoleLogs];
      const consoleLogText = allConsoleOutput.join('\n');
      
      // Check that console logs contain the first instance value
      if (!consoleLogText.includes(FIRST_INSTANCE_VALUE)) {
        throw new Error(`Console output does not contain expected value "${FIRST_INSTANCE_VALUE}". API consoleOutput: ${JSON.stringify(apiConsoleOutput)}, Browser console logs: ${JSON.stringify(consoleLogs)}`);
      }
      
      // Check that console logs contain object structure (should have properties)
      // The log should contain the instance data
      const hasObjectStructure = consoleLogText.includes('{') || consoleLogText.includes('[') || consoleLogText.toLowerCase().includes('instance');
      if (!hasObjectStructure) {
        throw new Error(`Console output does not show object structure. API consoleOutput: ${JSON.stringify(apiConsoleOutput)}, Browser console logs: ${JSON.stringify(consoleLogs)}`);
      }
      
      // 12. Verify no execution errors are displayed
      // (Already checked above with errorNotification)
      
      // 13. Verify function execution completes successfully
      // Wait for running state to clear (button should no longer be disabled)
      await page.waitForTimeout(1000);
      const runButtonAfter = page.locator('button:has-text("RUN"), button:has-text("Run")');
      await expect(runButtonAfter).toBeEnabled({ timeout: 10000 });
      
      // Verify execution completed by checking response was successful
